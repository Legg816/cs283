...
int get_student(int fd, int id, student_t *s){
    ...
    return NO_ERROR; // Student found and copied into *s
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t existing_student = {0}; // Buffer to check existing record
    off_t myoffset = id * STUDENT_RECORD_SIZE;
...
int print_db(int fd){
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        ...
        // Convert GPA and print student record
...
int compress_db(int fd){
    ...
    // Seek to the beginning of the current database
    if (lseek(fd, 0, SEEK_SET) < 0) {
        printf(M_ERR_DB_READ);
        close(new_fd);
        return ERR_DB_FILE;
    ...
    // Reopen the compressed database and return its new file descriptor
